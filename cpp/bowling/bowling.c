...
int main()
{
    int frameScore[10][3]={0, }; //10=9+1,3=2+1
    int frameScore10[4]={0, }; //4=3+1

    int  scores[11]; //11=10+1
    ...
    for(int i=1;i<=9;i++)
    {
        ...
        if(frameScore[i-1][1]!=10 && frameScore[i-1][1]+ frameScore[i-1][2]==10 
        && i>= 2){
             scores[i-1]= scores[i-2]+10+ frameScore[i][1];
             printf("frame: %d score: %d\n", i-1,scores[i-1]);

        }else if(frameScore[i-2][1]== 10 && frameScore[i-1][1]==10 
        && 1>=3){
             //(i-2) frame: strike (i-1) frame: strike: strike-> find i-2 frame score
             scores[i-2]= scores[i-3]+ 10+ 10+ frameScore[i][1];
             printf("#frame: %d score: %d\n",i-2, scores[i-2]);
        }

        //second bow <- frist bow  !=10
        if(frameScore[i][1] !=10) {
            ...
            //i frame: strike X, spare X-> find i frmae score
            }
    ...
    if(frameScore[9][1] !=10 && frameScore[9][1]+ frameScore[9][2]==10 ){
        scores[9]=scores[8]+10+frameScore[10][1];
        printf("#frame: %d score: %d\n",9, scores[9]);
